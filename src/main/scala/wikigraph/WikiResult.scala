...
case class WikiResult[A](value: Future[Either[Seq[WikiError], A]]):
  ...
  def map[B](f: A => B)(using ExecutionContext): WikiResult[B] =
    val result = this.value.map(e => e.map(v => f(v)))
    WikiResult(result)
  ...
  def flatMap[B](f: A => WikiResult[B])(using ExecutionContext): WikiResult[B] = 
    val futureB: Future[Either[Seq[WikiError], B]] = value.flatMap {
      eitherA => eitherA.map(f) match
        case Right(e) => e.value
        case Left(e) => Future.successful(Left[Seq[WikiError], B](e))
    }
    WikiResult(futureB)
...
object WikiResult:
  ...
  def traverse[A, B](as: Seq[A])(f: A => WikiResult[B])(using ExecutionContext): WikiResult[Seq[B]] =
    as.foldLeft[WikiResult[Seq[B]]](successful(Vector.empty)) {
      (futureValidatedElems, elem) =>
        futureValidatedElems.zip(f(elem)).map(t => t._1 :+ t._2)
    }
