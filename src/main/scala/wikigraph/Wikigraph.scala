...
final class Wikigraph(client: Wikipedia):
  ...
  def namedLinks(of: ArticleId): WikiResult[Set[String]] =
    val links = client.linksFrom(of)
    links.flatMap(artIds => WikiResult.traverse(artIds.toSeq)(client.nameOfArticle).map(_.toSet))
    ...
    def iter(visited: Set[ArticleId], q: Queue[(Int, ArticleId)]): WikiResult[Option[Int]] =
      if q.isEmpty then
       WikiResult.successful(None)
        ...
        case ((depth, _), _) if depth > maxDepth => WikiResult.successful(None)
        case ((depth, artId), remainingQueue) =>
          val neighbours: WikiResult[Set[ArticleId]] =
            client.linksFrom(artId).fallbackTo(WikiResult.successful(Set.empty))
          neighbours.flatMap {
            s => {
              if s.isEmpty then
                iter(visited, remainingQueue)
              else if s.contains(target) then
                WikiResult.successful(Some(depth))
              else
                val notVisited = s.filterNot(id => visited.exists(_ == id))
                if notVisited.isEmpty then
                  WikiResult.successful(None)
                else
                  val notVisitedTuple = notVisited.map(id => (depth + 1, id))
                  iter(visited ++ notVisited, remainingQueue.enqueueAll(notVisitedTuple))
            }
          }
  ...
  def distanceMatrix(titles: List[String], maxDepth: Int = 50): WikiResult[Seq[(String, String, Option[Int])]] =
    def convertToTuples(s: Seq[ArticleId]): Seq[(ArticleId, ArticleId)] =
      s.flatMap(e => s.filterNot(_ == e).map((e, _)))

    val result = WikiResult.traverse(titles)(client.searchId)
    result.flatMap {
      artIds =>
        WikiResult.traverse(convertToTuples(artIds))(t =>
          for {
            title1 <- client.nameOfArticle(t._1)
            title2 <- client.nameOfArticle(t._2)
            distance <- breadthFirstSearch(t._1, t._2, maxDepth)
          } yield (title1, title2, distance)
        )
    }
